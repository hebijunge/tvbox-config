...
class Spider(Spider):
    ...
    def _engine_lines(self, tag, disp, sp, title):
        ...
        try:
            ...
            try:
                ...
                res = sp.searchContent(title, '1')   # 兼容仅两参的搜索引擎
            ...
            for i, group in enumerate(groups):
                eflag = names[i].strip() if i < len(names) and names[i].strip() else '线路%d' % (i + 1)
                episodes = []
    ...
    def playerContent(self, flag, video_id, vipFlags):
        ...
        engine_degraded = False   # 引擎线路解析失败且无 URL 兜底时置真, 禁止再拿无效 id 硬解
        ...
        if '@@' in video_id:
            ...
            try:
                ...
                for t, _disp, sp in self._load_engines():
                    if t != tag:
                        continue
                    # 单引擎调用独立捕获, 一次抖动不牵连整条链路
                    r = None
                    try:
                        r = sp.playerContent(eflag, eid, vipFlags)
                        if isinstance(r, str):
                            r = json.loads(r)
                    except Exception:
                        r = None
                    if isinstance(r, dict) and r.get('url'):
                        # header 缺失时优先补引擎自身的 UA/Referer (4K 源站常严校 UA, 影探 UA 会被 403)
                        r.setdefault('header', getattr(sp, 'headers', self.headers))
                        r.setdefault('parse', 0)
                        r.setdefault('jx', 0)
                        r.setdefault('playUrl', '')
                        return r
                    # 引擎失败: eid 本身是 URL 时继续走下方案例兜底(可出流); 否则标记降级防死链
                    engine_degraded = not re.match(r'^https?://', eid or '')
                    video_id = eid
                    break
            except Exception:
        ...
        # 引擎解析失败且无任何可播地址 → 返回空, 让播放器明确提示失败(可切换其它线路), 不再拿无效 id 走 jx=1 白等
        if engine_degraded:
            return {'jx': 0, 'parse': 0, 'playUrl': '', 'url': '', 'header': self.headers}
