...
# 2026-09-26：拉丁词关键词改词边界匹配消费（见 norm_channel 注）；viu 单列正则防孤立词误命中
_HKTW_LATIN_WORDS = ("tvb", "tvbs", "hib", "uhb", "rthk")
_HKTW_VIU_RE = re.compile(r"\bviu(?:tv|\d|$)")
_HKTW_CJK_KW = tuple(k for k in HKTW_KW if not re.fullmatch(r"[a-z0-9]+", k))
...
# 带「香港/华视/tvb/中天(易中天)」字样的还被误归「港台」。显式标题表在 HKTW 关键词之前
# 判类（宁缺毋滥：只收实证出现过的房间名，新剧名轮播仍需个案补录）。键为去空格小写名。
LUNBO_EXACT = (
    "三国演义", "新三国", "蓝翔新三国", "西游记", "西游记后传", "天龙八部",
    "鹿鼎记", "封神榜", "射雕英雄传", "神雕侠侣", "倚天屠龙记", "大唐双龙传",
    "水浒传", "新白娘子传奇", "少年包青天第一部", "宰相刘罗锅", "康熙微服私访记",
    "回家的诱惑", "狂飙", "陈情令", "武林外传", "济公游记", "最美的青春", "闯关东",
    "爱情公寓", "中华小当家", "七龙珠", "火影忍者", "猫和老鼠", "海绵宝宝",
    "蜡笔小新", "仙剑奇侠传", "欢乐颂", "知否知否应是绿肥红瘦", "陈翔六点半",
    "二号交通站", "周润发", "成龙", "林正英", "燕双鹰", "易中天品三国",
    "欢乐集结号", "国内喜剧", "亮哥讲电影2", "经典动漫", "24h国漫热播",
    "千禧经典", "1930年代经典电影", "香港刑侦实录", "香港喜剧大全",
    "香港院线大片-最新", "华视-动作大片", "华视-喜剧大片", "华视-悬疑恐怖",
    "华视-爱情大片", "华视-高分好片", "tvb悬疑大片", "tvb练习生", "tvbaby",
    "itvb大后宫",
)
...
def norm_channel(name):
    ...
    if low.startswith(("cgtn", "cgtv")):
        return "CGTN", "央视"
    # 2026-09-26 修复（用户实证）：虎牙/YY 轮播房间名优先判归轮播（见 LUNBO_EXACT 注）
    if low in LUNBO_EXACT:
        return name.strip(), "轮播"
    # 2026-09-26 修复（用户实证）：拉丁词关键词按词边界匹配（在保留空格的小写名上）——
    # 旧逻辑在去空格名上做子串匹配，"TV Bahia / TV Brasil / MyCamTV Big" 的 "tvb" 全部
    # 误命中，巴西台/成人台混进港台组；"Já Viu TV"（巴西台）的孤立词 "viu" 同理。
    # viu 额外要求后跟 tv/数字/结尾（ViuTV/Viu6/VIU），孤立词不命中。
    low_sp = re.sub(r"\[[a-z0-9]{1,4}\]", "", n).lower()
    if any(k in low for k in _HKTW_CJK_KW) or any(
            re.search(r"\b" + re.escape(k), low_sp)
            for k in _HKTW_LATIN_WORDS) or _HKTW_VIU_RE.search(low_sp):
        return n, "港台"
    if "卫视" in low:
